...
if __name__ == "__main__":
    uvicorn.run(app, host="0.0.0.0", port=8000)connected_sockets = set()
binary_buffer = bytearray()
ble_trigger_reset = False
sk_client = None  # Global variable for the persistent connection pool
...
async def push_to_signal_k(path: str, value: float):
    """Pushes telemetry measurements using a shared, persistent connection pool"""
    global sk_client
    if sk_client is None:
        sk_client = httpx.AsyncClient()
    ...
    try:
        # Reuses existing sockets instantly over local loopback without building up connections
        await sk_client.post(url, json=delta_payload, timeout=0.03)
    except Exception:
        pass
...
async def lifespan(app: FastAPI):
    ...
    yield
    bg_task.cancel()
    # Closes the persistent Signal K network connection pool immediately on system exit
    if sk_client is not None:
        await sk_client.aclose()
